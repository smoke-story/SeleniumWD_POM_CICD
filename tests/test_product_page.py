...
class TestUserAddToBasketFromProductPage(BaseTest, LoginUser):
    ...
    @allure.title("user cant see success message")
...
class TestProductPageGuest(BaseTest):
    ...
    @allure.title("guest cant see success message after adding product to basket")
    @pytest.mark.xfail(reason="Bug in fixing process...")
    ...
    @allure.title("guest cant see product in basket opened from product page")
